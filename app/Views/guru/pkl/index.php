...
<div class="h-full">
    <div class="mb-4">
        <h1 class="text-2xl font-bold text-gray-800">Verifikasi Jurnal PKL</h1>
        <p class="text-sm text-gray-500">Guru Pembimbing PKL</p>
    </div>
        ...
        <div id="master-detail-container"
            ...
            <div id="detail-panel"
                ...
                <div id="empty-state"
                    ...
                    <p class="text-sm mt-1 text-gray-500">Pilih siswa di sebelah kiri untuk melihat dan memverifikasi jurnal
                        PKL</p>
                </div>
                ...
                <?php foreach ($groupedData as $student):
                    ...
                    <div id="student-detail-<?= $student['siswa_id'] ?>"
                        ...
                        <div class="flex-grow overflow-y-auto p-6 bg-gray-50/40 space-y-5 custom-scrollbar">
                            <div id="filter-placeholder-<?= $student['siswa_id'] ?>" class="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <div id="filter-loading-<?= $student['siswa_id'] ?>" class="hidden flex flex-col items-center justify-center py-12 text-center">
                                <div class="w-16 h-16 rounded-full bg-blue-50 text-blue-500 flex items-center justify-center mb-3">
                                    <i class="fas fa-spinner fa-spin text-2xl"></i>
                                </div>
                                <h3 class="text-base font-semibold text-gray-700">Memuat data...</h3>
                            </div>
                            <div id="filter-empty-<?= $student['siswa_id'] ?>" class="hidden flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <div id="filter-result-<?= $student['siswa_id'] ?>" class="hidden space-y-5"></div>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
...
<script>
    ...
    function selectStudent(siswaId, forceShowDetailOnMobile = true) {
        ...
        // Load week info for this student if not already loaded
...
<script>
    ...
    function getStatusBadge(status) {
        var badges = {
            ...
            'verified': {bg: 'bg-blue-50 text-blue-700 border-blue-200', label: 'Terverifikasi', icon: 'fa-check-double'},
    ...
    function renderProgressCard(p, siswaId) {
        ...
        if (p.catatan_instruktur || p.catatan_pembimbing) {
            ...
            if (p.catatan_pembimbing) {
                innerCat += '<div class="bg-orange-50 border-l-4 border-orange-500 rounded-r-xl p-3.5 shadow-sm"><p class="text-[10px] font-bold text-orange-700 uppercase tracking-wider flex items-center gap-1"><i class="fas fa-user-tie text-[8px]"></i> Catatan Pembimbing</p><p class="text-xs text-orange-900 mt-1 leading-relaxed">' + (p.catatan_pembimbing || '').replace(/</g, '&lt;') + '</p></div>';
            }
        ...
        if (p.status === 'approved') {
            actionHtml = '<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-green-50/40 border border-green-100 rounded-xl p-3.5 pl-4 shadow-inner"><div class="flex items-center gap-2.5 text-xs text-green-800"><i class="fas fa-check-circle text-base text-green-500 flex-shrink-0"></i><div class="min-w-0"><span class="font-bold">Progress disetujui</span>' + (p.catatan_pembimbing ? '<p class="text-green-700 mt-0.5 italic break-words">"' + (p.catatan_pembimbing || '').replace(/</g, '&lt;') + '"</p>' : '') + '</div></div><form action="' + BASE_URL + '/guru/jurnal-pkl/batal-verifikasi/' + p.id + '" method="POST" onsubmit="saveActiveSiswa(' + siswaId + ')"><input type="hidden" name="' + csrfName + '" value="' + csrfHash + '"><button type="submit" onclick="return confirm(\'Batalkan verifikasi progress ini?\')" class="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white border border-orange-200 text-orange-700 font-semibold rounded-xl hover:bg-orange-50 hover:border-orange-300 text-xs shadow-sm transition-all active:scale-95 whitespace-nowrap"><i class="fas fa-undo text-[10px]"></i> Batalkan</button></form></div>';
        } else if (p.status === 'verified' && p.verified_by) {
            actionHtml = '<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-blue-50/40 border border-blue-100 rounded-xl p-3.5 pl-4 shadow-inner"><div class="flex items-center gap-2.5 text-xs text-blue-800"><i class="fas fa-check-double text-base text-blue-500 flex-shrink-0"></i><div class="min-w-0"><span class="font-bold">Terverifikasi</span>' + (p.catatan_pembimbing ? '<p class="text-blue-700 mt-0.5 italic break-words">"' + (p.catatan_pembimbing || '').replace(/</g, '&lt;') + '"</p>' : '') + '</div></div><form action="' + BASE_URL + '/guru/jurnal-pkl/batal-verifikasi/' + p.id + '" method="POST" onsubmit="saveActiveSiswa(' + siswaId + ')"><input type="hidden" name="' + csrfName + '" value="' + csrfHash + '"><button type="submit" onclick="return confirm(\'Batalkan verifikasi progress ini?\')" class="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white border border-orange-200 text-orange-700 font-semibold rounded-xl hover:bg-orange-50 hover:border-orange-300 text-xs shadow-sm transition-all active:scale-95 whitespace-nowrap"><i class="fas fa-undo text-[10px]"></i> Batalkan</button></form></div>';
        } else if (p.status === 'revision' && (p.revision_requested_by === 'pembimbing' || p.revision_requested_by === 'both')) {
            if (p.revision_requested_by === 'both') {
                actionHtml = '<div class="flex items-center gap-2.5 text-xs text-orange-850 bg-orange-50/40 border border-orange-100 rounded-xl p-3.5 pl-4 shadow-inner"><i class="fas fa-exclamation-triangle text-base text-orange-500 flex-shrink-0"></i><div class="min-w-0"><span class="font-bold">Menunggu revisi dari siswa</span>' + (p.catatan_instruktur ? '<p class="text-orange-700 mt-0.5 italic break-words">"' + (p.catatan_instruktur || '').replace(/</g, '&lt;') + '"</p>' : '') + '</div></div>';
            } else {
                actionHtml = '<div class="flex items-center gap-2.5 text-xs text-orange-850 bg-orange-50/40 border border-orange-100 rounded-xl p-3.5 pl-4 shadow-inner"><i class="fas fa-exclamation-triangle text-base text-orange-500 flex-shrink-0"></i><div class="min-w-0"><span class="font-bold">Anda sudah meminta revisi</span>' + (p.catatan_pembimbing ? '<p class="text-orange-700 mt-0.5 italic break-words">"' + (p.catatan_pembimbing || '').replace(/</g, '&lt;') + '"</p>' : '') + '</div></div>';
            }
        } else if (p.status === 'revision' && p.revision_requested_by === 'instruktur' && p.verified_by) {
            // Instruktur requested revision, guru already verified — wait for student to upload revision
            actionHtml = '<div class="flex items-center gap-2.5 text-xs text-orange-850 bg-orange-50/40 border border-orange-100 rounded-xl p-3.5 pl-4 shadow-inner"><i class="fas fa-exclamation-triangle text-base text-orange-500 flex-shrink-0"></i><div class="min-w-0"><span class="font-bold">Menunggu revisi dari siswa</span>' + (p.catatan_instruktur ? '<p class="text-orange-700 mt-0.5 italic break-words">"' + (p.catatan_instruktur || '').replace(/</g, '&lt;') + '"</p>' : '') + '</div></div>';
        } else if (p.status === 'submitted' && p.verified_by) {
            // After student uploads revision, guru already has verification saved from before
            actionHtml = '<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-blue-50/40 border border-blue-100 rounded-xl p-3.5 pl-4 shadow-inner"><div class="flex items-center gap-2.5 text-xs text-blue-800"><i class="fas fa-check-double text-base text-blue-500 flex-shrink-0"></i><div class="min-w-0"><span class="font-bold">Terverifikasi</span>' + (p.catatan_pembimbing ? '<p class="text-blue-700 mt-0.5 italic break-words">"' + (p.catatan_pembimbing || '').replace(/</g, '&lt;') + '"</p>' : '') + '</div></div><form action="' + BASE_URL + '/guru/jurnal-pkl/batal-verifikasi/' + p.id + '" method="POST" onsubmit="saveActiveSiswa(' + siswaId + ')"><input type="hidden" name="' + csrfName + '" value="' + csrfHash + '"><button type="submit" onclick="return confirm(\'Batalkan verifikasi progress ini?\')" class="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white border border-orange-200 text-orange-700 font-semibold rounded-xl hover:bg-orange-50 hover:border-orange-300 text-xs shadow-sm transition-all active:scale-95 whitespace-nowrap"><i class="fas fa-undo text-[10px]"></i> Batalkan</button></form></div>';
        } else {
            actionHtml = '<form action="' + BASE_URL + '/guru/jurnal-pkl/verify/' + p.id + '" method="POST" onsubmit="saveActiveSiswa(' + siswaId + ')" class="space-y-3"><input type="hidden" name="' + csrfName + '" value="' + csrfHash + '"><div class="bg-gray-50 border border-gray-200 rounded-xl p-3.5 shadow-inner"><label class="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Catatan Pembimbing <span class="text-red-500">*</span></label><textarea name="catatan" rows="2" required maxlength="200" oninput="updateCharCount(this)" class="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-xs text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all resize-none shadow-sm" placeholder="Tulis catatan revisi atau catatan persetujuan..."></textarea><div class="text-right text-[10px] text-gray-400 char-count">0/200 karakter</div><div class="flex flex-col sm:flex-row justify-end gap-2.5 mt-3"><button type="submit" name="status" value="revision" onclick="return confirm(\'Minta revisi progress ini?\')" class="w-full sm:w-auto px-4 py-2 border border-orange-200 text-orange-700 font-bold text-xs hover:bg-orange-50 hover:border-orange-300 bg-white rounded-xl shadow-sm transition-all active:scale-95 flex items-center justify-center gap-1.5"><i class="fas fa-edit text-[10px]"></i> Minta Revisi</button><button type="submit" name="status" value="approved" onclick="return confirm(\'Setujui progress ini?\')" class="w-full sm:w-auto px-4 py-2 bg-blue-600 text-white font-bold text-xs hover:bg-blue-700 rounded-xl shadow-sm transition-all active:scale-95 flex items-center justify-center gap-1.5"><i class="fas fa-check text-[10px]"></i> Setujui</button></div></div></form>';
        }
        ...
        return '<div class="bg-white rounded-2xl border border-gray-200 p-4 md:p-5 shadow-sm space-y-4 md:space-y-6 hover:shadow-md transition-all duration-200">' +
            '<div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-4"><div class="min-w-0"><h3 class="text-base font-semibold text-gray-900 leading-snug">' + (p.task_judul || p.nama_task || '').replace(/</g, '&lt;') + '</h3><div class="mt-1.5 flex flex-wrap items-center gap-2 text-xs md:text-sm text-gray-500">' + kategoriHtml + '<span class="flex items-center gap-1"><i class="far fa-calendar"></i> ' + formatIndoDate(p.tanggal) + '</span>' + langkahCount + '</div></div><span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border self-start sm:self-auto flex-shrink-0 ' + badge.bg + '"><i class="fas ' + badge.icon + ' text-[10px]"></i> ' + badge.label + '</span></div>' +
            langkahHtml +
    ...
    function loadFilteredProgress(siswaId) {
        ...
        $.ajax({
            url: BASE_URL + '/guru/jurnal-pkl/filtered-progress/' + siswaId,
            type: 'GET',
    ...
    function loadTasks(siswaId, week) {
        ...
        $.ajax({
            url: BASE_URL + '/guru/jurnal-pkl/tasks/' + siswaId,
            type: 'GET',
            data: { week: week },
            dataType: 'json',
            success: function(response) {
    ...
    function loadWeekInfo(siswaId) {
        $.ajax({
            url: BASE_URL + '/guru/jurnal-pkl/week-info',
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.success && response.data.length > 0) {
