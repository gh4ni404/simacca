...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50/30 p-4 md:p-6 lg:p-8">
    <div class="max-w-3xl mx-auto">
        ...
        <form id="jurnalEditForm" action="<?= base_url('guru/jurnal-piket/update/' . $jurnal['id']) ?>" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <?= csrf_field() ?>
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="date" id="tanggal" name="tanggal" 
                            value="<?= esc(old('tanggal', $jurnal['tanggal'])) ?>" 
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4" 
                        placeholder="Tuliskan uraian kegiatan piket yang telah dilaksanakan..." 
                        ...
                        oninput="updateCharCount(this); clearFieldError(this)"><?= esc(old('deskripsi', $jurnal['deskripsi'])) ?></textarea>
                    ...
                    <div id="foto_dropzone" class="border-2 border-dashed <?= session('errors.foto_dokumentasi') ? 'is-field-error border-red-400 bg-red-50/30' : 'border-gray-200 bg-gray-50/50 hover:border-indigo-400' ?> rounded-2xl p-6 text-center transition">
                        ...
                        <div id="image_preview_container" class="hidden">
                            <div id="preview_grid" class="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-2xl mx-auto mb-4">
                                <!-- Previews (existing + new) rendered dynamically via JS -->
                            </div>
            ...
            <div class="p-6 bg-gray-50/80 border-t border-gray-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" id="btnSubmit" class="px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white text-sm font-semibold rounded-xl shadow-lg shadow-indigo-500/20 transition transform active:scale-95 inline-flex items-center gap-2">
                    <i class="fas fa-save"></i> Perbarui Jurnal Piket
                </button>
            </div>
        </form>
    </div>
...
let existingPhotos = <?= !empty($jurnal['foto_dokumentasi']) ? json_encode(explode(',', $jurnal['foto_dokumentasi'])) : '[]' ?>;
let selectedFiles = [];
...
function renderPreviews() {
    ...
    existingPhotos.forEach((photo, index) => {
        ...
        div.innerHTML = `
            <img src="<?= base_url('files/jurnal-piket/') ?>/${photo}" class="w-full h-full object-cover">
            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                <button type="button" onclick="removeExistingPhoto(${index})" class="h-8 w-8 rounded-lg bg-red-600 hover:bg-red-700 text-white flex items-center justify-center transition shadow-md">
    ...
    selectedFiles.forEach((file, index) => {
        ...
        reader.onload = function(e) {
            ...
            div.innerHTML = `
                ...
                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button type="button" onclick="removeNewFile(${index})" class="h-8 w-8 rounded-lg bg-red-600 hover:bg-red-700 text-white flex items-center justify-center transition shadow-md">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                </div>
            `;
...
document.getElementById('jurnalEditForm').addEventListener('submit', async function(e) {
    e.preventDefault();
    clearAllErrors();
    ...
    try {
        ...
        existingPhotos.forEach((photo) => {
            formData.append('keep_photos[]', photo);
        });
