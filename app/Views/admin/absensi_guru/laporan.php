        ...
        <form method="get" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div class="flex items-end space-x-2">
                ...
                <?php if (!empty($filters['tanggal_mulai']) || !empty($filters['tanggal_akhir']) || !empty($filters['guru_id']) || !empty($filters['status'])): ?>
                    <a href="<?= base_url('admin/absensi-guru/laporan') ?>" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition">
                        Reset
                    </a>
                <?php endif; ?>
            </div>
...
document.getElementById('btnExport').addEventListener('click', function() {
    ...
    window.location.href = `<?= base_url('admin/absensi-guru/export-excel') ?>?${queryString}`;
});
</script>
