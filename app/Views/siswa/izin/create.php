...
<div class="p-6">
    ...
    <div class="mb-6">
        <div class="flex items-center">
            <?= button_link('secondary', '', 'arrow-left', base_url('siswa/izin'), ['class' => 'mr-4 p-2']) ?>
            <div>
            ...
            <div class="bg-white rounded-lg shadow">
                <div class="p-4 md:p-6">
                    <form action="<?= base_url('siswa/izin/simpan'); ?>" method="POST" enctype="multipart/form-data" id="izinForm">
                        ...
                        <?= form_input('tanggal', 'Tanggal Izin', old('tanggal'), ['type' => 'date', 'required' => true, 'min' => date('Y-m-d')]) ?>

                        <?= form_select('jenis_izin', 'Jenis Izin', [
                            'Sakit' => 'Sakit',
                            'Izin' => 'Izin (Keperluan Keluarga/Lainnya)'
                        ], old('jenis_izin'), ['required' => true]) ?>

                        <?= form_textarea('alasan', 'Alasan', old('alasan'), [
                            'rows' => 4,
                            'required' => true,
                            'minlength' => 10,
                        ...
                        <div class="mb-6">
                            ...
                            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-blue-400 transition-colors">
                                <div class="space-y-1 text-center">
                                    ...
                                    <p class="text-xs text-gray-500">JPG, JPEG, PNG atau PDF (Max. 2MB, gambar dikompres otomatis)</p>
                        ...
                        <div class="flex gap-3">
                            <?= button('primary', 'Ajukan Izin', 'paper-plane', ['type' => 'submit', 'class' => 'flex-1 justify-center']) ?>
                            <?= button_link('secondary', 'Batal', 'times', base_url('siswa/izin'), ['class' => 'px-6 py-3']) ?>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="lg:col-span-1">
            ...
            <div class="bg-gradient-to-r from-yellow-400 to-orange-500 rounded-lg shadow p-6 text-white mb-6">
                ...
                <ul class="text-sm space-y-2 opacity-90">
                    <li class="flex items-start"><i class="fas fa-check mr-2 mt-1"></i><span>Ajukan izin minimal 1 hari sebelumnya</span></li>
                    <li class="flex items-start"><i class="fas fa-check mr-2 mt-1"></i><span>Sertakan dokumen untuk mempercepat proses</span></li>
                    <li class="flex items-start"><i class="fas fa-check mr-2 mt-1"></i><span>Jelaskan alasan dengan jelas dan sopan</span></li>
                </ul>
            </div>
    ...
    if (input.files && input.files[0]) {
        ...
        if (file.type.startsWith('image/')) {
            compressImage(file, function(compressedFile) {
                ...
                fileName.textContent = '✓ ' + file.name + ' (' + compSize + ' MB' + (compressedFile.size < file.size ? ' | dikompres)' : ')');
            });
        } else {
...
document.getElementById('izinForm').addEventListener('submit', function(e) {
    ...
    if (alasan.length < 10) { e.preventDefault(); alert('Alasan harus minimal 10 karakter!'); document.getElementById('alasan').focus(); return false; }
    if (!confirm('Apakah Anda yakin ingin mengajukan izin ini?')) { e.preventDefault(); return false; }
});
