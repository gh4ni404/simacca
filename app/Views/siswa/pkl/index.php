...
<div class="p-4 md:p-6">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden mb-6">
        <div class="px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-blue-500 to-blue-600">
            ...
            <p class="text-blue-100 text-sm"><?= date('d') . ' ' . $bulanIndo[(int)date('m')] . ' ' . date('Y') . ' &mdash; ' . $hariIndo[date('l')] ?></p>
        </div>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-shrink-0 flex items-center gap-1">
                        ...
                        <form action="<?= base_url('siswa/jurnal-pkl/kirim/' . $p['id']); ?>" method="POST" class="inline">
                            <?= csrf_field(); ?>
                            <button type="submit" title="Kirim untuk diverifikasi"
                                    class="px-2 py-1 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 text-xs transition-colors">
        ...
        <div class="space-y-3">
            ...
            <a href="<?= base_url('siswa/jurnal-pkl/hari/' . $day['tanggal']); ?>"
               class="block bg-white rounded-lg shadow hover:shadow-md transition-shadow overflow-hidden">
                <div class="px-5 py-4 flex items-center gap-4 <?= $isToday ? 'bg-blue-50 border-l-4 border-blue-500' : '' ?>">
                    <div class="flex-shrink-0 w-14 text-center">
                        <p class="text-xs text-gray-500"><?= $dayName ?></p>
                        <p class="text-2xl font-bold <?= $isToday ? 'text-blue-600' : 'text-gray-800' ?>"><?= $dateObj->format('d') ?></p>
                        <p class="text-xs text-gray-400"><?= $bulanIndo[(int)$dateObj->format('m')] ?></p>
                    </div>
                    <div class="flex-1">
                        <div class="flex items-center gap-2">
                            <span class="text-sm font-medium text-gray-800"><?= $day['total_aktivitas'] ?> aktivitas</span>
                            <?php if ($allApproved): ?>
    ...
    <div class="bg-white rounded-lg shadow p-5">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            <a href="<?= base_url('siswa/jurnal-pkl/cetak-jurnal/' . date('Y') . '/1'); ?>"
               target="_blank"
