...
<div class="hidden md:flex items-center gap-3 bg-gray-50 border border-gray-200 px-4 py-1.5 rounded-full shadow-sm">
    ...
    <div class="flex gap-4 text-center">
        ...
        <div>
            <span class="block text-xs font-bold text-yellow-600"><?= $totalStats['submitted'] + $totalStats['verified_by_instruktur'] ?></span>
    ...
    <div id="master-detail-container" class="flex flex-col lg:flex-row gap-6 lg:h-[calc(100vh-12rem)] lg:overflow-hidden">
        ...
        <div id="list-panel" class="w-full lg:w-80 bg-white rounded-2xl border border-gray-200 flex flex-col overflow-hidden shadow-sm flex-shrink-0 animate-fade-in">
            ...
            <div class="flex-grow overflow-y-auto divide-y divide-gray-100 custom-scrollbar">
                ...
                <button type="button" onclick="selectStudent(<?= $s['siswa_id'] ?>)"
                    ...
                    data-name="<?= strtolower(esc($s['nama_lengkap'])) ?>"
                    data-nis="<?= strtolower(esc($s['nis'])) ?>">
                    ...
                    <div class="relative flex-shrink-0">
                        ...
                        <?php else: ?>
                            <div class="w-10 h-10 rounded-full bg-indigo-50 text-indigo-600 border border-indigo-100 flex items-center justify-center font-bold text-sm shadow-sm">
        ...
        <div id="detail-panel" class="flex-grow bg-white rounded-2xl border border-gray-200 flex flex-col overflow-hidden shadow-sm lg:h-full min-h-[400px]">
            ...
            <div id="empty-state" class="flex-grow flex flex-col items-center justify-center p-12 text-center text-gray-500">
                ...
                <p class="text-sm mt-1 text-gray-500">Pilih siswa di sebelah kiri untuk melihat dan mereview jurnal PKL</p>
            </div>
            ...
            ?>
            <div id="student-detail-<?= $s['siswa_id'] ?>" class="student-detail-panel hidden flex flex-col h-full overflow-hidden">
                ...
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 flex-shrink-0">
                    <div class="flex items-center gap-3.5">
                        ...
                        <button type="button" onclick="backToList()" class="lg:hidden inline-flex items-center justify-center p-2.5 rounded-xl bg-white border border-gray-200 text-gray-600 hover:text-gray-900 shadow-sm transition-all hover:bg-gray-50">
                        ...
                        <?php if (!empty($s['profile_photo'])): ?>
                            <img src="<?= base_url('profile-photo/' . esc($s['profile_photo'])) ?>"
                                ...
                                alt="<?= esc($s['nama_lengkap']) ?>">
                        <?php else: ?>
                            <div class="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-lg border-2 border-white shadow-md">
                                <?= strtoupper(substr(esc($s['nama_lengkap']), 0, 2)) ?>
                            </div>
                        <?php endif; ?>
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1 text-xs text-gray-500">
                                <span class="font-medium text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded">NIS: <?= esc($s['nis']) ?></span>
                                <span class="text-gray-300">&bull;</span>
                                <span class="flex items-center gap-1 font-medium text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded"><i class="fas fa-school text-[10px]"></i> <?= esc($s['nama_kelas'] ?? '-') ?></span>
                            </div>
                        </div>
                    </div>

                    <div class="flex items-center gap-3">
                        <div class="text-xs bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-xl font-semibold border border-indigo-100 flex items-center gap-1.5 shadow-sm">
                ...
                <div class="flex-grow overflow-y-auto p-6 bg-gray-50/40 space-y-5 custom-scrollbar">
                    <?php if (empty($studentProgress)): ?>
                        <div class="flex flex-col items-center justify-center py-12 text-center">
                            <div class="w-16 h-16 rounded-full bg-green-50 text-green-500 flex items-center justify-center mb-3">
                            ...
                            <div class="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-4 hover:shadow-md transition-all duration-200">
                                ...
                                <div class="flex items-start justify-between gap-4 flex-wrap">
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div>
                                            <h5 class="text-sm font-bold text-gray-900 leading-snug"><?= esc($p['task_judul']) ?></h5>
                                            <div class="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1 text-xs text-gray-500">
                                                <span class="flex items-center gap-1"><i class="far fa-calendar text-gray-400"></i> <?= $formatIndoDate($p['tanggal']) ?></span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="pl-1">
                                    ...
                                    <ol class="space-y-1.5">
                                        ...
                                        <li class="flex items-start gap-2.5 text-xs text-gray-700">
                                            <span class="flex-shrink-0 w-5 h-5 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-[10px]"><?= $idx + 1 ?></span>
                                            <span class="pt-0.5 leading-snug"><?= esc($step) ?></span>
                                        </li>
                                        <?php endforeach; ?>
                                    </ol>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 pl-1">
                                    ...
                                    <div class="bg-blue-50/50 border border-blue-100 rounded-xl p-3 shadow-sm">
                                        ...
                                        <p class="text-xs text-blue-900 mt-1"><?= esc($p['catatan_pembimbing']) ?></p>
                                    </div>
                                    <?php endif; ?>
                                </div>
                                <?php endif; ?>
                                ...
                                <div class="pt-3 border-t border-gray-100">
                                    <?php if ($p['status'] === 'submitted'): ?>
                                        <form action="<?= base_url('instruktur/jurnal-pkl/verifikasi-progress/' . $p['id']) ?>" method="POST" class="space-y-3">
                                            <?= csrf_field() ?>
                                    ...
                                    <?php elseif (in_array($p['status'], ['verified_by_instruktur', 'revision'])): ?>
                                        <div class="flex items-center justify-between gap-4 bg-indigo-50/40 border border-indigo-100 rounded-xl p-3.5 pl-4 shadow-inner">
                                            <div class="flex items-center gap-2.5 text-xs text-indigo-800">
                                                ...
                                                <div class="min-w-0">
                                                    <span class="font-bold">Progress <?= $p['status'] === 'verified_by_instruktur' ? 'diverifikasi' : 'direvisi' ?></span>
                                                    <?php if (!empty($p['catatan_instruktur'])): ?>
                                                    <p class="text-indigo-700 mt-0.5 italic break-words">"<?= esc($p['catatan_instruktur']) ?>"</p>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                            <form action="<?= base_url('instruktur/jurnal-pkl/batal-verifikasi-progress/' . $p['id']) ?>" method="POST" onsubmit="saveActiveSiswa(<?= $s['siswa_id'] ?>)">
                                                <?= csrf_field() ?>
                                    ...
                                    <?php elseif ($p['status'] === 'approved'): ?>
                                        ...
                                        <div class="flex items-center justify-between gap-4 bg-green-50/40 border border-green-100 rounded-xl p-3.5 pl-4 shadow-inner">
                                            <div class="flex items-center gap-2.5 text-xs text-green-800">
                                                ...
                                                <div class="min-w-0">
                                                    ...
                                                    <?php if (!empty($p['catatan_instruktur'])): ?>
                                                    <p class="text-green-700 mt-0.5 italic break-words">"<?= esc($p['catatan_instruktur']) ?>"</p>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                            <form action="<?= base_url('instruktur/jurnal-pkl/batal-verifikasi-progress/' . $p['id']) ?>" method="POST" onsubmit="saveActiveSiswa(<?= $s['siswa_id'] ?>)">
                                        ...
                                        <?php else: ?>
                                        <div class="bg-gray-50 border border-gray-200 rounded-xl p-3.5 shadow-inner">
                                            <p class="text-[10px] font-bold text-green-600 uppercase tracking-wider mb-1.5 flex items-center gap-1">
...
<style>
    ...
    .student-item.active { background-color: #EEF2FF; border-color: #6366F1; }
    .student-item.active h4 { color: #4F46E5; }
</style>
